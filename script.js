...
const bookmarklets = [
    { name: "2048Solver", category: "Games", description: "Auto-plays 2048 using an expectimax AI. It reads the board, searches several moves ahead (keeping your biggest tile cornered), and presses the arrow keys for you — reaching the 2048 tile essentially every game and often 4096+. Works on classic-DOM versions like 2048game.com (the official play2048.co now uses an unreadable canvas). On 2048game.com, press an arrow once to start, then click the bookmarklet; click again to stop." },
    ...
    { name: "GoogleSnake", category: "Games", description: "Auto-plays Google's Snake game (search 'snake game' → Play). It follows a precomputed Hamiltonian cycle, reading only the snake's head from the canvas, so it never collides with itself — it survives indefinitely and keeps growing. Click Play, then click the bookmarklet (click again to stop). It's safe-but-steady, not speed-optimized." },
...
function makeCard(b) {
    ...
    // Draggable launcher. The bookmark's name when dragged = this link's text,
    // so the text must be the feature name (the star is a CSS ::before icon, not text).
    const launcher = document.createElement("a");
    launcher.className = "launcher";
    launcher.textContent = b.name;
